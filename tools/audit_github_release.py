...
NOTICE_TOKENS = (
    ...
    "credits", "patents", "notice", "attribution", "third-party", "third_party",
)
...
def resolve_tag_commit(repository_full_name: str, tag: str) -> dict:
    ...
    record = request_json(f"https://api.github.com/repos/{repository_full_name}/git/ref/tags/{encoded_tag}")
    ...
    if object_type == "commit":
        return {"ref": record.get("ref"), "tag_object": None, "commit": object_sha}
    if object_type != "tag":
        raise RuntimeError(f"Unsupported Git tag object type: {object_type!r}")
    tag_record = request_json(f"https://api.github.com/repos/{repository_full_name}/git/tags/{object_sha}")
    ...
    return {"ref": record.get("ref"), "tag_object": object_sha, "commit": target["sha"]}


def fingerprint_match(
    matches: list[tuple[str, bytes]], required_suffix: str, detector_md5_bytes: int | None = None
) -> dict:
    if len(matches) != 1:
        raise RuntimeError(
    ...
    name, raw = matches[0]
    if detector_md5_bytes is not None and detector_md5_bytes < 0:
        raise RuntimeError("detector_md5_bytes must be non-negative")
    detector_raw = raw if not detector_md5_bytes else raw[:detector_md5_bytes]
    return {
        "path": name,
        "size": len(raw),
        "md5": hashlib.md5(raw, usedforsecurity=False).hexdigest(),
        "sha256": hashlib.sha256(raw).hexdigest(),
        "detector_md5": hashlib.md5(detector_raw, usedforsecurity=False).hexdigest(),
        "detector_md5_bytes": detector_md5_bytes or 0,
    }


def inspect_zip(path: Path, required_suffix: str, detector_md5_bytes: int | None = None):
    members, notices, matches = [], [], []
    ...
    with zipfile.ZipFile(path, "r") as archive:
        for info in archive.infolist():
            ...
            members.append({
                "path": member_name, "size": info.file_size,
            ...
            if notice_name(member_name):
                ...
                notices.append({
                    "path": member_name, "size": info.file_size,
                    "sha256": hashlib.sha256(raw).hexdigest(), "text": decode_notice(raw),
                })
    return members, fingerprint_match(matches, required_suffix, detector_md5_bytes), notices


def inspect_tar(path: Path, required_suffix: str, detector_md5_bytes: int | None = None):
    members, notices, matches = [], [], []
    suffix = safe_path(required_suffix).as_posix().casefold()
    with tarfile.open(path, "r:*") as archive:
        for info in archive.getmembers():
            ...
            if notice_name(member_name):
                notices.append({
                    "path": member_name, "size": info.size,
                    "sha256": hashlib.sha256(raw).hexdigest(), "text": decode_notice(raw),
                })
    return members, fingerprint_match(matches, required_suffix, detector_md5_bytes), notices
...
def inspect_archive(
    path: Path, filename: str, required_suffix: str, detector_md5_bytes: int | None = None
):
    lower = filename.casefold()
    if lower.endswith(".zip"):
        return inspect_zip(path, required_suffix, detector_md5_bytes)
    if lower.endswith(".tar.gz") or lower.endswith(".tgz") or lower.endswith(".tar"):
        return inspect_tar(path, required_suffix, detector_md5_bytes)
...
def audit(manifest_path: Path, output_path: Path) -> dict:
    ...
    detector_md5_bytes = expected_payload.get("md5_bytes")
    if detector_md5_bytes is not None:
        detector_md5_bytes = int(detector_md5_bytes)
    ...
    release = request_json(f"https://api.github.com/repos/{repository}/releases/tags/{quote(tag, safe='')}")
    ...
    with tempfile.TemporaryDirectory(prefix="abandonware-github-release-") as temp_dir:
        ...
        members, payload, notices = inspect_archive(
            archive_path, expected_filename, required_suffix, detector_md5_bytes
        )
        archive_hash = sha256(archive_path)
        archive_size = archive_path.stat().st_size

    expected_md5 = str(expected_payload["md5"]).casefold()
    observed_detector_md5 = payload["detector_md5"].casefold()
    exact_match = (
        observed_detector_md5 == expected_md5
        and payload["size"] == int(expected_payload["size"])
    )

    record = {
        "schema": 2,
        "id": manifest["id"], "title": manifest.get("title"),
        "repository_full_name": repository, "release_tag": tag,
        "release_id": release.get("id"), "release_page": release.get("html_url"),
        "published_at": release.get("published_at"), "tag_identity": tag_identity,
        "asset": {
            "name": expected_filename, "id": asset.get("id"), "size": archive_size,
            "sha256": archive_hash, "download_url": asset_url,
        },
        "required_payload": payload, "expected_payload": expected_payload,
        "payload_matches_expected_identity": exact_match,
        "member_count": len(members), "members": members, "notices": notices,
        "decision": "IDENTITY_MATCH_REQUIRES_RIGHTS_REVIEW" if exact_match else "IDENTITY_MISMATCH",
        "warning": (
            "This is a non-publishing identity audit. Detector MD5 may intentionally cover only "
            "the configured prefix used by ScummVM's Advanced Detector. A matching author-hosted "
            "release payload does not itself authorize redistribution; production still requires "
            "the candidate rights decision and all third-party exceptions to pass."
        ),
    }
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(record, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(json.dumps({
        "id": record["id"], "repository": repository, "tag": tag,
        "tag_commit": tag_identity["commit"], "asset": expected_filename,
        "asset_size": archive_size, "asset_sha256": archive_hash,
        "payload": payload, "payload_matches_expected_identity": exact_match,
        "notice_paths": [notice["path"] for notice in notices], "decision": record["decision"],
    }, indent=2))
    if not exact_match:
        raise RuntimeError(f"GitHub author release payload does not match expected identity for {manifest['id']}")
    return record
